...
public interface UserProgressRepository extends JpaRepository<UserProgress, UUID> {
    ...
    Page<UserProgress> findByUserIdOrderByAnsweredAtDesc(UUID userId, Pageable pageable);

    @Query("""
                SELECT new com.example.demo.dto.response.AnswerHistoryResponse(
                    up.answerLogId,
                    q.title,
                    q.genre,
                    q.language,
                ...
                ORDER BY up.answeredAt DESC
            """)
    Page<AnswerHistoryResponse> findHistoryDto(UUID userId, Pageable pageable);
    ...
    List<Question> findIncorrectQuestionsWithCursor(
            ...
            @Param("difficulty") Integer difficulty, // ← 追加
            Pageable pageable);
    ...
    @Query(value = """
                ...
                WHERE (:difficulty IS NULL OR q.difficulty_level = :difficulty)
                GROUP BY q.language, q.genre
            """, nativeQuery = true)
    List<GenreStatsProjection> getGenreStats(UUID userId, Integer difficulty);
